function [dist,imgr,vh,vi] = imageReconstruction_v3(csvBaseline,csvAlt,marbleRemovalNum, ...
    meshComplexity, imagePrior, hyperSet, fixedNFValue, nodalSolve)
...
% BigDaddyClumped NOSER prior with lambda = 0.1402
% - For Nov20ThreeClumped2Sheet1.csv, fixedNF gave hyperSet = 0.0182
% 11-24_3Clumped_1 best hyperSet was 4.5917e-04 using fixedNF method
...
%meshString = sprintf('%s2d1c',meshChoice); % gives electrodes some
%thickness to them
...
% Read in data. Detect if the data format is .csv; if so, use csvread and
% if not, use textread.
if(regexp(csvBaseline,'.csv'))
    baseline = csvread(csvBaseline);
else
    baseline = textread(csvBaseline);
end

if(regexp(csvAlt,'.csv'))
    config1 = csvread(csvAlt);
else
    config1  = textread(csvAlt);
end

% Make sure data matrices are 20x20 arrays (sometimes artifacts were
% introduced and made a 21st column of zeros);
baseline = baseline(:,[1:20]);
config1 = config1(:,[1:20]);
for(i=1:20)
    baseline(i,i)=0;
    config1(i,i)=0;
end
...
for (i = 1:length(nElec))
    imdl.fwd_model.electrode(i).z_contact = [zElec(i)];
end
...
elseif(hyperSet=='none')
    % Nothing. Uses lambda from priors set above.
end
...
%% Plotting
figure(1); clf
% Call marbleRemovalPicture function to show regions of marble removal and
% generate subplot
marbleRemovalPicture(marbleRemovalNum,meshComplexity);

% Subplot solved image
subplot(1,2,2)
show_fem(imgr)
titleString = sprintf('Reconstruction, Amp = %0.3f, %s Stimulation, %s Measure',...
...
centroids = ginput(2);
dist = pdist(centroids) % Euclidean distance is default